...
CMD="sniper -m masswebscan -w \"$WORKSPACE\" -t \"$TARGETS\""
echo "[*] Running: $CMD" 2>&1 | tee -p -a sniper-collection-${DATE}.txt
eval "$CMD" 2>&1 | tee -p -a sniper-collection-${DATE}.txt

CMD="sniper -m massvulnscan -w \"$WORKSPACE\" -t \"$TARGETS\""
echo "[*] Running: $CMD" 2>&1 | tee -p -a sniper-collection-${DATE}.txt
eval "$CMD" 2>&1 | tee -p -a sniper-collection-${DATE}.txt
